...
/// </summary>
public static class ServiceCollectionGenericAssertions
{
    /// <summary>
    ...
    public static void AssertServiceRegistration(this ServiceCollection services, IAssertionAdaptor assert,
        ServiceDescriptorSpecification serviceSpecification
    )
    {
        IEnumerable<ServiceDescriptor> query = services.Where(serviceSpecification.Criteria);
        IReadOnlyList<ServiceDescriptor> found = query.ToList();
        
        assert.True(found.Count == serviceSpecification.RegistrationCount, 
            $"Expected {serviceSpecification.RegistrationCount} but found {found.Count} for {serviceSpecification.ToString()}.");
    }
    ...
    public static void AssertServiceRegistration(this ServiceCollection services, IAssertionAdaptor assert,
        Type serviceType, ServiceLifetime specificLifetime, Type implementationType, 
    ...
    {
        IReadOnlyList<ServiceDescriptor> found = services.Where(x =>
            x.ServiceType == serviceType && x.ImplementationType == implementationType).ToList();
